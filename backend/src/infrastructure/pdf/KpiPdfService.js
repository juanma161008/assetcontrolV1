...
const toLines = (report = {}) => {
  const lines = [
    "MICROCINCO & CIA LTDA - REPORTE KPI MENSUAL",
    "",
    `Periodo: ${report.periodo || "-"}`,
    `Generado: ${report.generado_en || "-"}`,
    "",
    `MTBF (dias): ${report.mtbf ?? "-"}`,
    `MTTR (horas): ${report.mttr ?? "-"}`,
    `Disponibilidad (%): ${report.disponibilidad ?? "-"}`,
    `OEE (%): ${report.oee ?? "-"}`,
    "",
    `Mantenimientos totales: ${report.total_mantenimientos ?? "-"}`,
    `Finalizados: ${report.finalizados ?? "-"}`,
...
export default class KpiPdfService {
  async generar(reportData = {}) {
    return buildPdfBuffer(toLines(reportData));
  }
}
